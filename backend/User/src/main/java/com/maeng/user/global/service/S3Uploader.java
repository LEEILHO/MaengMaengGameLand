package com.maeng.user.global.service;
...
public class S3Uploader {
	...
	public String upload(MultipartFile multipartFile, String dirName) throws IOException {
		File uploadFile = convert(multipartFile).orElseThrow(() -> new UserException(UserExceptionCode.FAIL_TO_EDIT_PROFILE));
	...
	private String upload(File uploadFile, String dirName) {
		...
		String newFileName = dirName + "/" + UUID.randomUUID().toString().replace("-", "") + extension;
	...
	public void deleteFile(String fileName) {
		try {
			...
			log.info("이전 S3 파일 삭제 성공: " + fileName);
		} catch (Exception e) {
